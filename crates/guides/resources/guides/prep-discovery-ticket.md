...
Before writing, perform a lightweight intent challenge. Look for missing or ambiguous **user-owned intent** that could cause reasonable independent Discovery agents to proceed under materially different assumptions about behavior, scope, compatibility, allowed change, acceptance boundaries, or other consequential user decisions. Ask focused questions only when the answers could materially improve their common starting point. This is not a completeness checklist or fixed questionnaire; zero questions is valid. Stop asking when no material user-intent ambiguity remains.
